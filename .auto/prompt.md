...
- Fixed in the self-closing file-tag iteration: image `<file>` tag extraction now recognizes both paired `<file ...></file>` and self-closing `<file ... />` forms, so the whole image marker is removed from hover display.
