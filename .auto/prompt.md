...
- Next hypothesis: rich clipboard / issue content can include HTML image tags such as `<img src="/tmp/shot.png" alt="bug">`. Thumbnail extraction already sees the local `src`, but hover display may leave broken `<img src= alt=...>` markup unless the whole tag is normalized to useful alt text.
