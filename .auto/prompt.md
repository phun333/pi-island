...
- Next hypothesis: pi's path UX often uses `@path` syntax. `@/tmp/shot.png` and `@./shot.png` should render as image thumbnails but the hover prompt should not leave a stray `@` after removing the path.
