...
- Next hypothesis: HTML `<picture>` markup may include `<source srcset="/tmp/shot.png 1x">` or `<source src="/tmp/shot.png">` entries. Thumbnail extraction can see these paths, but hover display may leave broken `<source ...>` markup unless local image source tags are removed.
