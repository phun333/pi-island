...
- Fixed in the HTML-figure-local-image iteration: figure and figcaption wrapper tags are stripped from hover display after local image normalization, preserving readable alt/caption text.
- Next hypothesis: broaden direct-image de-dupe coverage across equivalent textual forms (`file://`, percent-encoded paths, query/fragment suffixes, Markdown/HTML wrappers), reference-style Markdown variants, and escaped/encoded rich clipboard markup to ensure same-content references de-dupe without suppressing unrelated attachments.
