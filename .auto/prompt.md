...
- Fixed in the direct+HTML-srcset iteration: HTML candidate groups now check all grouped local paths for a direct-image content hash before adding the first path-derived thumbnail. This suppresses the whole semantic group when any candidate mirrors a direct image.
