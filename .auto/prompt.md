...
- Next hypothesis: rich HTML image markup may use `srcset` instead of `src` (`<img srcset="/tmp/shot.png 1x" alt="bug">`). Thumbnail extraction can see the path, but hover display may leave broken `<img srcset=...>` markup unless `srcset` is treated like a local image source.
