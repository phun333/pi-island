...
- Next hypothesis: users may paste Markdown image syntax such as `![bug](/tmp/screenshot.png)` or `![bug](</tmp/Screen Shot.png>)`. The image path should still render as a thumbnail, and hover text should not show leftover Markdown punctuation like `![bug]`; keeping the alt text is useful.
