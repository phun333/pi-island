...
- Next hypothesis: HTML `<picture>` blocks combine `<source>` and `<img>` alternatives for one semantic image. The island should show one thumbnail for the whole picture and hover display should keep the `<img alt>` text without leaving `<picture>` wrapper markup.
