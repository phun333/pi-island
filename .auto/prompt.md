...
- Next hypothesis: duplicate raw references to the same image path may appear in different textual forms (literal spaces and shell-escaped spaces). Thumbnail extraction should still de-dupe by resolved path, but hover display should remove every raw occurrence, not only the first de-duped match.
