...
- Next hypothesis: Markdown local-image references can appear as normal links (`[screenshot](/tmp/a.png)`) or use filenames containing parentheses (`![shot](/tmp/Screen Shot (1).png)`). The current simple Markdown cleanup may stop at the first `)` and leave `![shot]`/`[shot]` clutter even though thumbnail extraction succeeds.
