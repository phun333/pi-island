...
- Relative-to-temp paths initially passed, but that test was weak because `../../var/...` can be recognized by matching an absolute suffix. Strengthened workload now uses a cwd-contained `./.auto/tmp-rel-images-*/relative Screen Shot.png` so success requires true relative-path extraction and display elision.
