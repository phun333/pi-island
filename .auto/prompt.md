...
- Next hypothesis: users may type or path-complete relative local image paths (`./shot.png`, `../tmp/shot.png`) rather than absolute `/var/...` paths. Since pi tools resolve relative paths against `cwd`, pi-island should render explicit relative image paths too and hide them from hover text.
