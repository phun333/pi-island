...
- Next hypothesis: some clipboard/browser flows produce percent-encoded local paths without a `file://` prefix, e.g. `/tmp/Screen%20Shot.png`. These should resolve to the real file with spaces and be hidden from hover text, while still allowing literal percent filenames as fallback.
