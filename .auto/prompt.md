...
- Fixed in the same-content raw-copy iteration: direct-image hash matching is now a non-consuming membership check, so every same-content path fallback is suppressed while different-content raw images still render separately.
