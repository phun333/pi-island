...
- Next hypothesis: mixed prompt inputs can include a direct pasted image plus a separate CLI/file-argument image tag. The current direct-image/file-tag de-dupe should skip only tags that actually mirror the direct image, not every file tag when any direct image exists.
