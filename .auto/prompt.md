...
- Next hypothesis: pi CLI/file-argument image attachments put a `<file name="/path/to/image.png"></file>` marker in prompt text while also passing `evt.images`. pi-island should not duplicate thumbnails/counts in that case, and the hover display should not show empty `<file>` markup after hiding the local path.
