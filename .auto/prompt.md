...
- Next hypothesis: local image paths copied from browser/devtools may include query strings or fragments (`/tmp/shot.png?cache=123#frag`). Thumbnail extraction can resolve the base file, but hover display should remove the whole URL-ish token and Markdown cleanup should preserve clean alt text.
