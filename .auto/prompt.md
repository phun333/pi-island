...
- Fixed in the HTML-entity-attributes iteration: `normalizePromptImagePath()` now falls back to HTML-entity-decoded paths after literal/percent lookup, and `htmlAttrValue()` decodes common entities so alt text displays as `R&D` instead of `R&amp;D`.
