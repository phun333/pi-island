...
- Fixed in the direct-input-image iteration: `input_image` payloads now use the same local/data URL direct attachment path and content-hash de-dupe as `image_url` objects, without fetching remote URLs.
- Next hypothesis: broaden direct-image de-dupe coverage across equivalent textual forms (`file://`, percent-encoded paths, query/fragment suffixes, Markdown/HTML wrappers) and escaped/encoded rich clipboard markup to ensure same-content references de-dupe without suppressing unrelated attachments.
