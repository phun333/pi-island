...
- Next hypothesis: duplicate Markdown references to the same local image path (for example before/after labels pointing at the same pasted temp image) should not duplicate thumbnails, but display cleanup must still remove every Markdown wrapper, not just the first occurrence of a de-duplicated path.
