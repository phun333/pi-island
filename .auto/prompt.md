...
- Next hypothesis: some paste/attach flows can pass a direct `ImageContent` payload while the prompt text also contains the same raw local temp path (not wrapped in a `<file>` tag). This should not create duplicate thumbnails, while a direct image plus a different raw local image path should still show both.
