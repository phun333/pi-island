...
- Next hypothesis: HTML `srcset` can include multiple local candidates for the same semantic image (`1x`, `2x`). The island should show one thumbnail/tile for that semantic attachment, not one per density candidate, while hover display still keeps clean alt text.
